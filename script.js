        ...
        const API_URL =
          window.location.hostname === 'localhost'
            ...
            : 'https://shedlike-cyndi-unapprovingly.ngrok-free.dev/api/auth';
        ...
        function speak(text) {
            if ('speechSynthesis' in window) {
                ...
                utterance.lang = 'en-US'; // Set language for the voice
        ...
        // This is the new function to activate the voice assistant
        function toggleVoiceAssistant() {
            if (annyang) {
                // Check if it's already listening
                if (annyang.isListening()) {
                    annyang.abort(); // Stop listening if clicked again
                } else {
                    ...
                    <div class="bg-white rounded-2xl p-6 card-shadow">
                        ...
                        <div class="space-y-3">
                            <div class="p-3 border border-gray-200 rounded-xl">
                                ...
                                <div class="text-sm text-gray-600">0.8 km away</div>
                                <div class="text-xs text-green-600 mb-2">✅ Open 24/7</div>
                                <button onclick="callStore('911234567890')" class="w-full bg-green-500 text-white py-2 rounded-lg text-sm hover:bg-green-600">
                                    📞 Call Now
                                </button>
                            </div>
                            <div class="p-3 border border-gray-200 rounded-xl">
                                ...
                                <button onclick="callStore('910987654321')" class="w-full bg-green-500 text-white py-2 rounded-lg text-sm hover:bg-green-600">
                                    📞 Call Now
                                </button>
                            </div>
                        </div>
        ...
        function callStore(phoneNumber) {
            alert(`This button will call the store at ${phoneNumber}. Feature coming soon!`);
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (annyang) {
                // Define all the voice commands
                const commands = {
                    'help': () => { speak('Opening emergency confirmation.'); showEmergencyConfirm(); },
                    'emergency': () => { speak('Opening emergency confirmation.'); showEmergencyConfirm(); },
                    'show reminders': () => { speak('Showing your reminders.'); showReminders(); },
                    'show my medicine': () => { speak('Showing your medicine reminders.'); showReminders(); },
                    'find hospitals': () => { speak('Finding nearby places.'); showNearbyPlaces(); },
                    'show nearby places': () => { speak('Finding nearby places.'); showNearbyPlaces(); },
                    'find a nurse': () => { speak('Showing available nurses.'); showFindNurse(); },
                    'check health status': () => { speak('Opening your health status.'); showHealthStatus(); },
                    'show my health': () => { speak('Opening your health status.'); showHealthStatus(); },
                    'show caretakers': () => { speak('Showing your linked caretakers.'); showCaretakers(); },
                    'show my records': () => { speak('Opening your documents.'); showUploadRecords(); },
                    'show documents': () => { speak('Opening your documents.'); showUploadRecords(); },
                    'go home': () => { speak('Going back to the home screen.'); showHome(); },
                    'show home screen': () => { speak('Going back to the home screen.'); showHome(); }
                };
            
                // Add the commands to the voice recognition engine
                annyang.addCommands(commands);
            
                // --- Add visual and audio feedback ---
                const voiceBtn = document.getElementById('voiceAssistantBtn');

                // When listening starts, make the mic icon red and pulse
                annyang.addCallback('start', () => {
                    if (voiceBtn) {
                        voiceBtn.querySelector('i').style.color = '#ef4444'; // Red color
                        voiceBtn.querySelector('i').classList.add('fa-beat');
                    }
                    speak('How can I help you?');
                });
            
                // When listening stops, return the mic icon to normal
                annyang.addCallback('end', () => {
                    if (voiceBtn) {
                        voiceBtn.querySelector('i').style.color = 'white';
                        voiceBtn.querySelector('i').classList.remove('fa-beat');
                    }
                });
